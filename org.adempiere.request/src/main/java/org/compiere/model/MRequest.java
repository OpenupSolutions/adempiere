...
import static org.compiere.model.X_M_Inventory.DOCSTATUS_Completed;
...
//		}
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (status != null)
		{
			...
			if (status.isFinalClose()){
				...
				if(!isProcessed()){
					...
					//se genera inventario de uso interno
					//Openup. Nicolas Sarlabos. #10860.
					//generateIventory();
					//Fin #10860.
		...
		//Openup. Nicolas Sarlabos. 28/11/2018. #11009.
		if (update.isNewInfo() || this.getM_ProductSpent_ID() > 0)
			update.saveEx();
		else
			update = null;
		//Fin #11009.
	...
	/**
	 * OpenUp. Nicolas Sarlabos. #10860. 07/09/2018.
	 * Metodo que genera un inventario de uso interno al cerrarse la solicitud.
	 */
	/*private void generateIventory() {

		MRequestUpdate[] updates = this.getUpdates(null);
		...
		if(updates.length > 0){
			...
			MOrg org = new MOrg(getCtx(), this.getAD_Org_ID(), get_TrxName());
			MOrgInfo info = org.getInfo();

			//se recorren lineas de actualizacion de la solicitud
			for (int i = 0; i < updates.length; i++){

				MRequestUpdate ru = updates[i];

				if(ru.getM_ProductSpent_ID() > 0){

					if(inv == null){//creo cabezal la primera vez

						if (info.getM_Warehouse_ID() <= 0)
							throw new AdempiereException("La organización " + org.getName() + " no tiene almacén definido");

						MWarehouse warehose = (MWarehouse) info.getM_Warehouse();
						locator = MLocator.getDefault(warehose);

						if (locator == null)
							throw new AdempiereException("No se obtuvo ubicación predeterminada para el almacén '" + warehose.getName() + "'");

						Integer docID = MDocType.getDocType("MMI", this.getAD_Org_ID());

						//se crea cabezal del documento
						inv = new MInventory(getCtx(), 0 , get_TrxName());
						inv.setM_Warehouse_ID(info.getM_Warehouse_ID());
						inv.setMovementDate(today);
						inv.setC_DocType_ID(docID);
					...
					iLine.setM_Locator_ID(locator.get_ID());
					iLine.setM_Product_ID(ru.getM_ProductSpent_ID());
					iLine.setQtyInternalUse(ru.getQtySpent());
					iLine.saveEx();
			...
			if (inv != null) {
				...
				if (!DOCSTATUS_Completed.equals(inv.getDocStatus())) {
					m_processMsg = "@M_Inventory_ID@: " + inv.getProcessMsg();
...
*/
	...
	public void sendNotices(ArrayList<String> list)
	{
		...
		final String sql = "SELECT u.AD_User_ID, u.NotificationType, u.EMail, u.Name, MAX(r.AD_Role_ID) "
				...
				+ " UNION "
				+ " SELECT u.AD_User_ID, u.NotificationType, u.EMail, u.Name, MAX(r.AD_Role_ID)"
				+ " FROM AD_User u "
				+ " JOIN AD_User_Roles r ON u.AD_User_ID = r.AD_User_ID "
				+ " WHERE u.AD_User_ID=?"
				+ " GROUP BY u.AD_User_ID, u.NotificationType, u.EMail, u.Name";
